...
const Common = () => {
  ...
  const generateQuestionsmore = async () => {
    ...
    try {
      const response = await axios.get(
        `${process.env.REACT_APP_BACKEND_URL}/api/question/common`
      );

      setQuestionsList((prev) => {
        const updated = [...prev, ...(response.data.question || [])];
        localStorage.setItem("comon_questions", JSON.stringify(updated));
        return updated;
      });
    } catch (err) {
  ...
  useEffect(() => {
    const savedQuestions = localStorage.getItem("comon_questions");
    if (savedQuestions) {
      setQuestionsList(JSON.parse(savedQuestions));
    } else {
      setQuestionsList(questiondata["General Interview Questions"] || []);
    }

    const savedAnswers = localStorage.getItem("comon_answers");
    if (savedAnswers) {
  ...
  const fetchAnswer = async (question) => {
    ...
    try {
      ...
      setAnswers((prev) => {
        ...
        localStorage.setItem("comon_answers", JSON.stringify(updated));
        return updated;
      });
