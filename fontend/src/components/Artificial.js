...
const generateQuestionsmore = async () => {
    ...
    try {
        ...
        const response = await axios.get(
            `${process.env.REACT_APP_BACKEND_URL}/api/question/ai`,
          
        );
        console.log(response.data.question);
       setQuestionsList((prev) => {
  const updated = [...prev, ...(response.data.question || [])];
  localStorage.setItem("ai_questions", JSON.stringify(updated));
  return updated;
});
        setLoading(false);
...
useEffect(() => {
  const savedQuestions = localStorage.getItem("ai_questions");
  if (savedQuestions) {
    setQuestionsList(JSON.parse(savedQuestions));
  } else {
    setQuestionsList(questiondata["Artificial Intelligence"] || []);
  }
 ...
 const toggleAnswerVisibility = (question) => {
  ...
  localStorage.setItem("ai_answers", JSON.stringify(updated));
  return updated;
});
setVisibleAnswers((prev) => ({
        ...prev,
