...
const Os = () => {
  ...
  const generateQuestionsmore = async () => {
    ...
    try {
      const response = await axios.get(
        `${process.env.REACT_APP_BACKEND_URL}/api/question/os`
      );

      setQuestionsList((prev) => {
        const updated = [...prev, ...(response.data.question || [])];
        localStorage.setItem("os_questions", JSON.stringify(updated));
        return updated;
      });
    } catch (err) {
  ...
  useEffect(() => {
    const savedQuestions = localStorage.getItem("os_questions");
    if (savedQuestions) {
      setQuestionsList(JSON.parse(savedQuestions));
    } else {
      setQuestionsList(questiondata["Operating Systems"] || []);
    }

    const savedAnswers = localStorage.getItem("os_answers");
    if (savedAnswers) {
  ...
  const fetchAnswer = async (question) => {
    ...
    try {
      ...
      setAnswers((prev) => {
        ...
        localStorage.setItem("os_answers", JSON.stringify(updated));
        return updated;
      });
  ...
  return (
    <>
      ...
      <VantaBackgroundWrapper>
        <div className="container-fluid py-5 px-4" style={{overflowY: "auto",overflowX:"auto"}}><VantaBackgroundWrapper>
                ...
                >
                  <div className="d-flex justify-content-between align-items-center flex-wrap">
                    ...
                    <div className="d-flex gap-3">
                      ...
                      >
                        {answers[q] ? (visibleAnswers[q] ? "Hide Answer" : "Show Answer") : "Get Answer"}
                      </button>
                      <button
