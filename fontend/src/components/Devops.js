...
const Devops = () => {
  ...
  const generateQuestionsmore = async () => {
    ...
    try {
      const response = await axios.get(
        `${process.env.REACT_APP_BACKEND_URL}/api/question/devops`
      );
  ...
  useEffect(() => {
    const savedQuestions = localStorage.getItem("dev_questions");
    if (savedQuestions) {
      setQuestionsList(JSON.parse(savedQuestions));
    } else {
      setQuestionsList(questiondata["DevOps"] || []);
    }

    const savedAnswers = localStorage.getItem("dev_answers");
    if (savedAnswers) {
  ...
  const fetchAnswer = async (question) => {
    ...
    try {
      ...
      setAnswers((prev) => {
        ...
        localStorage.setItem("dev_answers", JSON.stringify(updated));
        return updated;
      });
          ...
          >
            ...
            >
              <h3 className="my-4 text-center">
                Devops/Cloud Related Interview Questions
              </h3>
