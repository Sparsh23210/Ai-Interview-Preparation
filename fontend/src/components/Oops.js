...
const Oops = () => {
  ...
  const generateQuestionsmore = async () => {
    ...
    try {
      const response = await axios.get(
        `${process.env.REACT_APP_BACKEND_URL}/api/question/oops`
      );

      setQuestionsList((prev) => {
        const updated = [...prev, ...(response.data.question || [])];
        localStorage.setItem("oops_questions", JSON.stringify(updated));
        return updated;
      });
    } catch (err) {
  ...
  useEffect(() => {
    ...
    if (savedQuestions) {
      ...
      setQuestionsList(questiondata["Object Oriented Programming"] || []);
    }

    const savedAnswers = localStorage.getItem("oops_answers");
    if (savedAnswers) {
  ...
  const fetchAnswer = async (question) => {
    ...
    try {
      ...
      setAnswers((prev) => {
        ...
        localStorage.setItem("oops_answers", JSON.stringify(updated));
        return updated;
      });
