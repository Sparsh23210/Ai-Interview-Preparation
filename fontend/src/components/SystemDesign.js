...
const SystemDesign = () => {
  ...
  const generateQuestionsmore = async () => {
    ...
    try {
      const response = await axios.get(
        `${process.env.REACT_APP_BACKEND_URL}/api/question/sd`
      );

      setQuestionsList((prev) => {
        const updated = [...prev, ...(response.data.question || [])];
        localStorage.setItem("sd_questions", JSON.stringify(updated));
        return updated;
      });
    } catch (err) {
  ...
  useEffect(() => {
    const savedQuestions = localStorage.getItem("sd_questions");
    if (savedQuestions) {
      setQuestionsList(JSON.parse(savedQuestions));
    } else {
      setQuestionsList(questiondata["System Design"] || []);
    }

    const savedAnswers = localStorage.getItem("sd_answers");
    if (savedAnswers) {
  ...
  const fetchAnswer = async (question) => {
    ...
    try {
      ...
      setAnswers((prev) => {
        ...
        localStorage.setItem("sd_answers", JSON.stringify(updated));
        return updated;
      });
